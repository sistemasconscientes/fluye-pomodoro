...
const CATEGORY_CONFIG: Record<Category, { label: string; emoji: string; color: string }> = {
  fix: { label: "Fix", emoji: "🐛", color: "bg-destructive/15 text-destructive" },
  improve: { label: "Mejora", emoji: "⚡", color: "bg-accent/15 text-accent-foreground" },
  new: { label: "Nuevo", emoji: "✨", color: "bg-primary/15 text-foreground" },
  design: { label: "Diseño", emoji: "🎨", color: "bg-secondary text-foreground" },
};
...
function getSessionVotes(): Record<string, "up" | "down"> {
  try {
    return JSON.parse(sessionStorage.getItem(VOTES_KEY) || "{}");
  } catch {
    return {};
  }
}
...
const Features = () => {
  ...
  const vote = (featureId: string, direction: "up" | "down") => {
    ...
    // Can't vote same direction twice
    if (existing === direction) return;

    const delta = direction === "up" ? 1 : -1;
    // If switching vote, apply double delta
    const actualDelta = existing ? delta * 2 : delta;

    const updated = features.map((f) =>
      f.id === featureId ? { ...f, votes: f.votes + actualDelta } : f
    );
    setFeatures(updated);
    saveFeatures(updated);
    setSessionVote(featureId, direction);
    setSessionVotesState(getSessionVotes());
  };
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-2xl flex-col px-5 py-8">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="font-display text-2xl text-foreground">Sugerencias</h1>
          <p className="text-sm text-muted-foreground">
            Vota y sugiere mejoras para Fluye
          </p>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showForm ? (
          ...
          >
            <div className="rounded-2xl bg-secondary/50 p-5 space-y-3">
              <input
                ...
                className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-primary/30 placeholder:text-muted-foreground/60"
                autoFocus
              />
                  ...
                  ([key, config]) => (
                    <button
                      ...
                      className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-all ${
                        newCategory === key
                          ? `${config.color} ring-2 ring-primary/20`
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 rounded-xl bg-primary py-2.5 text-sm font-medium text-primary-foreground transition-opacity hover:opacity-90 disabled:opacity-40"
                >
                  Agregar
                </button>
                <button
                  onClick={() => setShowForm(false)}
                  className="rounded-xl border border-border px-4 py-2.5 text-sm text-muted-foreground transition-colors hover:bg-secondary/50"
                >
                  Cancelar
                </button>
              </div>
          ...
          <motion.button
            ...
            className="mb-6 flex w-full items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-border py-4 text-sm font-medium text-muted-foreground transition-colors hover:border-primary/40 hover:text-foreground"
          >
            <Plus size={16} />
            Sugerir nueva mejora
          </motion.button>
        )}
      </AnimatePresence>
        ...
        {sortedFeatures.map((feature, i) => {
          ...
          return (
            ...
            >
              ...
              {/* Category badge */}
              <span
                className={`flex items-center gap-1 rounded-lg px-2.5 py-1 text-[10px] font-medium shrink-0 ${config.color}`}
              >
                <Tag size={10} />
                {config.label}
              </span>
